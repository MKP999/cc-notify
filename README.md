...
## 已知限制：通知横幅图标
macOS 14+ 出于防伪造，**不给未用 Developer ID 签名/公证的 App 在通知横幅里显示自定义图标**——本地自建的 App（ad-hoc 签名）横幅左边会是空白方块。这是系统限制，不是 bug。
- **菜单栏铃铛**用的是完整图标，不受影响。
- 横幅**标题前加了 🔔 emoji**（如「🔔 Claude ▸ myproject」），即使图标空白也能一眼认出。
- 要真正的横幅图标，需 Apple Developer Program（$99/年）签发 Developer ID 并公证。
